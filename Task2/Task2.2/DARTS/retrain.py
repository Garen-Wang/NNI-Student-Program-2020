# modified from nni darts retrain
import logging
import time
import argparse
...
def train(config, train_loader, model, optimizer, criterion, epoch):
    ...
    for step, (x, y) in enumerate(train_loader):
        ...
        accuracy = getAccuracy(logits, y, topk=(1, 5))
...
if __name__ == "__main__":
    ...
    dataset_train, dataset_valid = getDatasets()

    model = DartsStackedCells(32, 3, 36, 10, args.layers)
    ...
    for epoch in range(args.epochs):
        ...
        if top1 > best_top1:
            best_top1 = top1
            if top1 > 0.97:
                torch.save(model.state_dict(), 'best_architecture_97.pth')
